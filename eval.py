...
# wnut_bio = ('B', 'I', 'O')
# wnut_tags = ('LOCATION','ORGANIZATION','PERSON')
...
def evaluate(fileName):
    ...
    input = open(fileName, encoding='utf-8').read().strip().split('\n')
    tokens = doc_to_toks(input)
    entities = doc_to_entities(input)
...
corpus: Corpus = ColumnCorpus(data_folder, columns,
                              train_file='emerging.train.conll',
                              test_file='emerging.testSubstituted.conll.txt',
                              dev_file='emerging.dev.conll')
result, score = tagger.evaluate(corpus.test, mini_batch_size=32,out_path='./result/Wnut-17-result.tsv')
resetSub('./result/Wnut-17-result.tsv')
evaluate('./result/Wnut-17-result.tsv')